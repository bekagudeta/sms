    ...
    public function generateSchedule($semesterId)
    {
        ...
        try {
            ...
            if (!$finalSchedule || empty($finalSchedule)) {
                Log::warning('No schedule generated by main AutoScheduler; attempting immediate fallback engine.');
            ...
            $result['day_distribution'] = $this->getDayDistributionStats();
            return $result;
            ...
            try {
                ...
                if ($engineResult['success'] || !empty($engineResult['assignments'])) {
                    ...
                    foreach ($engineResult['assignments'] as $assignment) {
                        // Verify this assignment belongs to the current semester
                        $section = Section::with('courseOffering')->find($assignment['section_id']);
                        if ($section && $section->courseOffering->semester_id == $this->semesterId) {
                            Schedule::create([
